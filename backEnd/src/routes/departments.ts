...
router.post("/", async (req, res) => {
  try {
    ...
    if (!createdDepartments) throw error;

    res.status(201).json({ data: createdDepartments });
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const [subjectCount, classesCount, enrolledStudentsCount] = await Promise.all([
      db
        .select({ count: sql<number>`count(*)` })
        .from(subjects)
    ...
    res.status(200).json({
      data: {
        department,
        totals: { subjects: subjectCount[0]?.count ?? 0, classes: classesCount[0]?.count ?? 0 },
